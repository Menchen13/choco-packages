...
$releases = 'https://api.github.com/repos/dorssel/usbipd-win/releases/latest'
...
function global:au_GetLatest {
    ...
    $url64 = $release.assets | Where-Object { $_.name -match '_x64\.msi$' } | Select-Object -ExpandProperty browser_download_url
    $urlArm64 = $release.assets | Where-Object { $_.name -match '_arm64\.msi$' } | Select-Object -ExpandProperty browser_download_url

    # 3. Calculate the ARM64 checksum manually (AU framework only auto-calculates x86/x64)
    $tempFile = Join-Path $env:TEMP "usbipd-win_$version`_arm64.msi"
